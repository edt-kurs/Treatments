...
Функция МВВБАкцииЦеныЗагрузкаЦен() Экспорт
	...
	Выборка = ПолучитьВыборкуТекущихАкций();
	
	Пока Выборка.Следующий() Цикл
			
		Путь = ПолучитьПуть(Выборка);
		Если Путь = Ложь Тогда
			Продолжить;
		КонецЕсли;
		...
		ОбработатьИЗаписатьЦены(Выборка, СтрокаТела);
		
	КонецЦикла;
	
	// Обновим доллар
	ПолучитьИЗаписатьЦенуДоллару();
	ПолучитьИЗаписатьЦенуЮань();
...
Функция ПолучитьВыборкуТекущихАкций()
...
Процедура ОбработатьИЗаписатьЦены(Выборка, СтрокаТела)
...
Процедура ПолучитьИЗаписатьЦенуДоллару()
	...
	//Путь = СтрШаблон("iss/statistics/engines/currency/markets/fixing/CNYFIXME.json?from=%1", ДатаЦены);
	HTTPЗапрос = Новый HTTPЗапрос(Путь, Заголовки);
	Результат = HTTPСоединение.Получить(HTTPЗапрос);
	СтрокаТела = Результат.ПолучитьТелоКакСтроку(КодировкаТекста.UTF8);
	//ДанныеJSON = ОбработатьСтрокуJSON1(СтрокаТела);
	...
	Если ЗначениеЗаполнено(СоответствиеЦенДоллар) Тогда
		РегистрыСведений.ЦеныАкцииНовый.Записать(СоответствиеЦенДоллар);
	КонецЕсли;
	
КонецПроцедуры

Процедура ПолучитьИЗаписатьЦенуЮань()
	...
	МассивЦен = МассивДанных["history"]["data"];
	КолСтрок = МассивДанных["history"]["data"].Количество()-1;
	СоответствиеЦенДоллар["Акция"] = Справочники.Акции.НайтиПоНаименованию("CNY/RUB");
	СоответствиеЦенДоллар["open"] = МассивЦен[КолСтрок][2];
	СоответствиеЦенДоллар["close"] =  МассивЦен[КолСтрок][2];
	//СоответствиеЦенДоллар["high"] =  МассивЦен[КолСтрок][2];
	//СоответствиеЦенДоллар["low"] =  МассивЦен[КолСтрок][2];
